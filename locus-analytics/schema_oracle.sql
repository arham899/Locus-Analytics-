...
CREATE OR REPLACE TRIGGER trg_audit_config
...
VALUES (
           ...
           'UPDATE',
           :NEW.admin_id,
           TO_CLOB(:OLD.config_id || ' | ' || :OLD.db_host || ' | ' || :OLD.google_maps_api_key),
           TO_CLOB(:NEW.config_id || ' | ' || :NEW.db_host || ' | ' || :NEW.google_maps_api_key),
           CURRENT_TIMESTAMP
       );
END;
/
