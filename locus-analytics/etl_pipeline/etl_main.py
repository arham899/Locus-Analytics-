...
for i, item in enumerate(listings):
    ...
    cur.execute("""
    INSERT INTO property (property_id, city, locality, property_type, area, price)
    VALUES (%s, %s, %s, %s, %s, %s)
    ON CONFLICT (property_id) DO UPDATE SET
        price = EXCLUDED.price,
        updated_at = CURRENT_TIMESTAMP
    """, (property_id, "Lahore", title, ptype, 10, price))
    
    progress = int((i + 1) / len(listings) * 100)
    cur.execute("""
    UPDATE etl_job SET progress_percent = %s, updated_at = CURRENT_TIMESTAMP
    WHERE job_id = %s
    """, (progress, job_id))

# cur.execute("""
# UPDATE etl_job SET status = 'SUCCESS', progress_percent = 100 
# WHERE job_id = %s
# """, (job_id,))
